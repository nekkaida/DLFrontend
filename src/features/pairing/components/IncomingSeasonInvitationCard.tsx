...
import type { SeasonInvitation } from '../hooks/useIncomingSeasonInvitations';

const { width } = Dimensions.get('window');
const isSmallScreen = width < 375;

interface IncomingSeasonInvitationCardProps {
  invitation: SeasonInvitation;
  onAccept: (invitationId: string) => void;
  onDeny: (invitationId: string, senderName: string) => void;
  isLoading?: boolean;
}

/**
 * Card component for displaying an incoming season invitation
 * Used in ManagePartnershipScreen when user has INCOMPLETE partnership
 * and another player (without partnership) has invited them
 */
export const IncomingSeasonInvitationCard: React.FC<IncomingSeasonInvitationCardProps> = ({
  invitation,
  onAccept,
  onDeny,
  isLoading = false,
  ...
  const handleDeny = () => {
    ...
    onDeny(invitation.id, sender.name);
  };
    ...
    <View style={styles.card}>
      ...
      <View style={styles.playerRow}>
        ...
        <View style={styles.playerInfo}>
          <Text style={styles.playerName}>{sender.name}</Text>
          {sender.displayUsername && (
            <Text style={styles.playerUsername}>@{sender.displayUsername}</Text>
          )}
